...
int Graph::smallestEdgeDFS(int currVertex, vector<string> &dfs)
{
    if(edgesDiscovered(currVertex) != graph.at(currVertex).edgeList.size())
    {
        ...
        while (compIndex < size)
        {
            ...
            else
            {
                ...
                {
                    if (graph.at(currVertex).edgeList.at(smallestIndex).weight >= graph.at(currVertex).edgeList.at(compIndex).weight)
                    {
    ...
    {
        vector<string>::iterator dfsIt = find(dfs.begin(), dfs.end(), graph.at(currVertex).city);
